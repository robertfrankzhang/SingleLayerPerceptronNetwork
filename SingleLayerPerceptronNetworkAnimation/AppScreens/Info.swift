...
class InfoScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let slideFour = SKLabelNode(position: CGPoint(x:self.frame.width*0.1+3*self.frame.width,y:self.frame.height-30-self.frame.width/16-80), zPosition: 1, text: "After the model trains, you can swipe between the neural network view and the Euclidean view to see what the neural network looks like at that point. After training, you can keep placing dots. If you press play again, the network will retrain to reoptimize the weights and biases to fit the new dots. You can also go into testing mode where you tap any point on the Euclidean grid and the app will tell you that point's color classification", fontColor: ThemeColor.darkPurple, fontName: "Antipasto Pro", fontSize: 20, verticalAlignmentMode: .center, horizontalAlignmentMode: .left)
